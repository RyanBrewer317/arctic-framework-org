...
-type state(OXB, OXC, OXD) :: {state,
        gleam@erlang@process:subject(action(OXC, OXD)),
        OXB,
        fun((OXB, OXC) -> {OXB, lustre@effect:effect(OXC)}),
        fun((OXB) -> lustre@internals@vdom:element(OXC)),
        lustre@internals@vdom:element(OXC),
        gleam@dict:dict(binary(), fun((lustre@internals@patch:patch(OXC)) -> nil)),
        gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, OXC} |
            {error, list(gleam@dynamic:decode_error())})),
        gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, OXC} |
            {error, list(gleam@dynamic:decode_error())}))}.

-type action(OXE, OXF) :: {attrs, list({binary(), gleam@dynamic:dynamic_()})} |
    {batch, list(OXE), lustre@effect:effect(OXE)} |
    {debug, debug_action()} |
    {dispatch, OXE} |
    {emit, binary(), gleam@json:json()} |
    {event, binary(), gleam@dynamic:dynamic_()} |
    {set_selector, gleam@erlang@process:selector(action(OXE, OXF))} |
    shutdown |
    {subscribe, binary(), fun((lustre@internals@patch:patch(OXE)) -> nil)} |
    {unsubscribe, binary()}.
...
-spec run_renderers(
    gleam@dict:dict(any(), fun((lustre@internals@patch:patch(OYK)) -> nil)),
    lustre@internals@patch:patch(OYK)
) -> nil.
run_renderers(Renderers, Patch) ->
    gleam@dict:fold(Renderers, nil, fun(_, _, Renderer) -> Renderer(Patch) end).

-spec run_effects(
    lustre@effect:effect(OYP),
    gleam@erlang@process:subject(action(OYP, any()))
) -> nil.
...
-spec loop(action(OXU, OXV), state(OXY, OXU, OXV)) -> gleam@otp@actor:next(action(OXU, OXV), state(OXY, OXU, OXV)).
loop(Message, State) ->
    case Message of
        {attrs, Attrs} ->
        ...
        {debug, {force_model, Model@2}} ->
            Model@3 = gleam@dynamic:unsafe_coerce(Model@2),
...
-spec start(
    ...
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, OXH} |
        {error, list(gleam@dynamic:decode_error())}))
) -> {ok, gleam@erlang@process:subject(action(OXH, any()))} |
