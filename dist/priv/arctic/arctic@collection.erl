...
-spec with_parser(
    arctic:collection(),
    fun((binary()) -> {ok, arctic:page()} | {error, snag:snag()})
) -> arctic:collection().
...
-spec default_parser() -> fun((binary()) -> {ok, arctic:page()} |
    {error, snag:snag()}).
default_parser() ->
    fun(Src) ->
        Parser = begin
            _pipe = arctic@parse:new(),
