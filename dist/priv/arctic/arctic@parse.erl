...
-type parse_result(UNM) :: {parse_result, UNM, list(parse_error())}.

-type parse_error() :: {parse_error, position(), binary()}.

-type arctic_parser(UNN) :: {arctic_parser,
        fun((binary(), parse_data(UNN)) -> parse_result(gleam@option:option({lustre@internals@vdom:element(nil),
            UNN})))}.

-opaque parse_data(UNO) :: {parse_data,
        position(),
        gleam@dict:dict(binary(), binary()),
        UNO}.
...
-type inline_rule(UNP) :: {inline_rule,
        binary(),
        binary(),
        fun((lustre@internals@vdom:element(nil), list(binary()), parse_data(UNP)) -> {ok,
                {lustre@internals@vdom:element(nil), UNP}} |
            {error, snag:snag()})}.

-type prefix_rule(UNQ) :: {prefix_rule,
        binary(),
        fun((lustre@internals@vdom:element(nil), parse_data(UNQ)) -> {ok,
                {lustre@internals@vdom:element(nil), UNQ}} |
            {error, snag:snag()})}.

-type component(UNR) :: {static_component,
        binary(),
        fun((list(binary()), binary(), parse_data(UNR)) -> {ok,
                {lustre@internals@vdom:element(nil), UNR}} |
            {error, snag:snag()})} |
    {dynamic_component, binary()}.

-opaque parser_builder(UNS) :: {parser_builder,
        list(inline_rule(UNS)),
        list(prefix_rule(UNS)),
        list(component(UNS)),
        UNS}.
...
-spec get_state(parse_data(UNZ)) -> UNZ.
get_state(Data) ->
    erlang:element(4, Data).

-spec with_pos(parse_data(UOB), position()) -> parse_data(UOB).
with_pos(Data, Pos) ->
    {parse_data, Pos, erlang:element(3, Data), erlang:element(4, Data)}.

-spec with_state(parse_data(UOE), UOE) -> parse_data(UOE).
with_state(Data, State) ->
    {parse_data, erlang:element(2, Data), erlang:element(3, Data), State}.

-spec new(UOH) -> parser_builder(UOH).
new(Start_state) ->
...
-spec add_inline_rule(
    ...
    binary(),
    binary(),
    fun((lustre@internals@vdom:element(nil), list(binary()), parse_data(UOJ)) -> {ok,
            {lustre@internals@vdom:element(nil), UOJ}} |
        {error, snag:snag()})
) -> parser_builder(UOJ).
...
-spec add_prefix_rule(
    parser_builder(UOR),
    binary(),
    fun((lustre@internals@vdom:element(nil), parse_data(UOR)) -> {ok,
...
-spec add_static_component(
    parser_builder(UOY),
    binary(),
    fun((list(binary()), binary(), parse_data(UOY)) -> {ok,
            {lustre@internals@vdom:element(nil), UOY}} |
        {error, snag:snag()})
) -> parser_builder(UOY).
...
-spec add_dynamic_component(parser_builder(UPF), binary()) -> parser_builder(UPF).
...
) -> fun((lustre@internals@vdom:element(nil), any(), parse_data(UWP)) -> {ok,
        {lustre@internals@vdom:element(nil), UWP}} |
    {error, any()}).
wrap_inline(W) ->
    fun(El, _, Data) -> {ok, {W([], [El]), get_state(Data)}} end.

-spec wrap_inline_with_attributes(
    fun((list(lustre@internals@vdom:attribute(UPS)), list(lustre@internals@vdom:element(nil))) -> lustre@internals@vdom:element(nil)),
    list(lustre@internals@vdom:attribute(UPS))
) -> fun((lustre@internals@vdom:element(nil), any(), parse_data(UWW)) -> {ok,
        {lustre@internals@vdom:element(nil), UWW}} |
    {error, any()}).
...
-spec wrap_prefix(
    fun((list(lustre@internals@vdom:attribute(any())), list(lustre@internals@vdom:element(nil))) -> lustre@internals@vdom:element(nil))
) -> fun((lustre@internals@vdom:element(nil), parse_data(UXD)) -> {ok,
        {lustre@internals@vdom:element(nil), UXD}} |
    {error, any()}).
wrap_prefix(W) ->
    fun(El, Data) -> {ok, {W([], [El]), get_state(Data)}} end.

-spec wrap_prefix_with_attributes(
    fun((list(lustre@internals@vdom:attribute(UQI)), list(lustre@internals@vdom:element(nil))) -> lustre@internals@vdom:element(nil)),
    list(lustre@internals@vdom:attribute(UQI))
) -> fun((lustre@internals@vdom:element(nil), parse_data(UXJ)) -> {ok,
...
-spec invert_res({ok, {VCJ, VCC}} | {error, VCG}, parse_data(VCC)) -> {{ok, VCJ} |
        {error, VCG},
    parse_data(VCC)}.
invert_res(Res, D) ->
    case Res of
...
-spec parse_component(list(component(USF))) -> arctic_parser(USF).
...
-spec parse_inline_rule(list(inline_rule(URB)), parse_data(URB)) -> party:parser(fun((parse_data(URB)) -> {{ok,
            lustre@internals@vdom:element(nil)} |
        {error, snag:snag()},
    parse_data(URB)}), snag:snag()).
...
-spec parse_markup(
    list(inline_rule(URO)),
    party:parser(nil, snag:snag()),
    parse_data(URO)
) -> party:parser({ok, {lustre@internals@vdom:element(nil), parse_data(URO)}} |
...
-spec parse_text(list(inline_rule(URZ)), list(prefix_rule(URZ))) -> arctic_parser(URZ).
